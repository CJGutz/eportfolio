...
const SkillCard = ({ id, skill }: { id: string; skill: SkillType }) => {
  const skillPercent = (skill.level!! / 5) * 100;

  return (
    <div
      id={id}
      className="flex flex-col gap-5 justify-center bg-off-white p-3 shadow-xl rounded-tl-xl rounded-br-xl"
    >
      <div className="grid grid-cols-1 h-full md:grid-cols-3 gap-6 items-center">
        <div className="flex flex-wrap justify-center flex-row md:flex-col gap-6">
          <h3 className="text-3xl text-center font-bold break-keep">
            {skill.name}
          </h3>
          {skill.icon && (
            <Image
        ...
        <div className="md:col-span-2 flex flex-col h-full">
          <p className="mb-4 text-sm grow">{skill.description}</p>
          {skill.level && (
