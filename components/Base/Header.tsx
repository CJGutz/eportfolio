    ...
    <header className="bg-transparent z-30">
      <nav className="container mx-auto p-6 lg:flex lg:items-center lg:justify-between">
        ...
        <div
          ...
          } inset-x-0 w-full bg-transparent px-6 py-4 shadow-md transition-all duration-200 ease-in-out lg:relative lg:top-0 lg:mt-0 lg:flex lg:w-auto lg:translate-y-0 lg:items-center lg:bg-transparent lg:p-0 lg:opacity-100 lg:shadow-none`}
        >
          <div className="flex flex-col space-y-4 lg:mt-0 lg:flex-row lg:space-y-0">
            {links.map(({ name, href }) => (
              <Link
                key={name}
                className="text-sm font-medium text-gray-700 hover:text-blue-500 lg:mx-4 lg:mvn-button"
                href={href}
              >
                {name}
              </Link>
